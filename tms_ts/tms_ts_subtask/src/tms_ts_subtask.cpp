...
bool tms_rp::TmsRpSubtask::move(SubtaskData sd)
{
  ...
  while (1)
  {
    ...
    case 2003: // smartpal5_2
    {
      ...
      if(0)//for simple demo
      {
        unity_robot_srv.request.unit = UNIT_VEHICLE;
        unity_robot_srv.request.cmd = CMD_MOVE_ABS;
        unity_robot_srv.request.arg.resize(3);
        unity_robot_srv.request.arg[0] = rp_srv.request.goal_pos.x;
        unity_robot_srv.request.arg[1] = rp_srv.request.goal_pos.y;
        unity_robot_srv.request.arg[2] = rp_srv.request.goal_pos.th;
        if(!sp5_unity_control_client.call(unity_robot_srv))
        {
          ROS_ERROR("False is returned from unity\n");
        }
        double arg_test[3];
        arg_test[0] = rp_srv.request.goal_pos.x;
        arg_test[1] = rp_srv.request.goal_pos.y;
        arg_test[2] = rp_srv.request.goal_pos.th;
        if (!sp5_control(sd.type, UNIT_VEHICLE, CMD_MOVE_ABS, 3, arg_test))
        {
          send_rc_exception(1);
          return false;
        }
      ...
      if (sd.type)
      {
             ...
             (rp_srv.response.VoronoiPath[i - 1].th >= -PI && rp_srv.response.VoronoiPath[i - 1].th < -HALF_PI)))
        {
          ...
          unity_robot_srv.request.unit = UNIT_VEHICLE;
          unity_robot_srv.request.cmd = CMD_MOVE_REL;
          unity_robot_srv.request.arg.resize(3);
          for(int i=0; i<unity_robot_srv.request.arg.size(); i++)
          {
            unity_robot_srv.request.arg[i] = tmp_arg[i];
          }
          if(!sp5_unity_control_client.call(unity_robot_srv))
          {
            ROS_ERROR("False is returned from unity\n");
